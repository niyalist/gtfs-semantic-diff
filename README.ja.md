...
**[diff.gtfs.jp](https://diff.gtfs.jp/)** で公開運用中 — ブラウザから事業者・世代を
選ぶ (または zip をアップロードする) だけでレポートを生成できる (無料・登録不要)。
digest・ID 対応表・JSON API・MCP サーバー等の機械可読データも提供
([開発者向けページ](https://diff.gtfs.jp/developers.html))。構成は S3 + CloudFront +
Lambda のサーバレス (`infra/` に AWS CDK 定義、運用手順は [docs/ops/](docs/ops/)) で、
自分の AWS アカウントへのデプロイも可能。
...
explained_ratio 1.0000、pytest 276件。国際検証セット (TriMet・MBTA・STM・ローマ・
スイス/オランダ国家規模) も完走 (explained 0.98〜1.0)。富山地鉄の令和8年4月1日改正では公式告知の
