...
V3 (ビューア再構築 — 4部構成 UI、検証モード、色弱原則の徹底) 進行中。
そのレビューで方向グループ規則のモデル欠陥 (端点完全一致要求) が判明し、
**V2.1 (方向グループ規則の一般化 — 順序整合度判定) を割り込み実施中**
(経緯と DoD は roadmap.md V2.1、規則は presentation.md R15 改訂注記)。
