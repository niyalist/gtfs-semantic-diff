...
GtfsSnapshot ×2
  ...
  → 説明台帳: 未消費 RawDiff → UNEXPLAINED_RESIDUAL (§5)
```
...
| `irregular` | 特定日運行 (下記の calendar_dates 判定、または下記 SD1 の低密度判定) |
| `inactive` | 運行日なし (全フラグ 0 かつ追加運行日なし = 休止中の定義枠。SD1: 実効運行日ゼロも含む) |

**SD1 (2026-07-23、docs/design/service_days.md §2.1)**: 曜日フラグを持つ service
も、calendar に start_date/end_date があれば**実効運行日**を数えて分類を上書き
しうる。実効運行日 = (期間 × 曜日フラグ − calendar_dates 削除 + 追加) を
フィード有効期間 (feed_info の start/end、無ければリポジトリ世代メタ、どちらも
無ければクリップなし) で切ったもの。

- 実効運行日 **ゼロ** → `inactive` (期限切れ service の残骸。PRT の旧期 W/U/S、
  便が載っていても便数に算入しない)
- **密度** (実効運行日 ÷ 期間×フラグ該当日) が `[load.day_types]
  min_flag_day_ratio` (0.5) **未満** → `irregular` (米国流の祝日専用 service:
  土曜フラグ+通常土曜を全削除で「祝日1日だけ」を表す型。PRT 実測 0.06)

日数でなく**密度**で判定する理由: 期間分割された正規ダイヤ (STM の四半期
period、世代同居の残存窓) は日数が少なくても密度 0.9〜1.0 であり、これを
特定日に誤分類しないため。日本の検証フィードでは分類は一切変化しない
(祝日 service はフラグを持たない流儀のため — docs/verification/SD1_day_types.md)。
...
平日等の時刻表に混入するのを防ぐ — 永井運輸の実例から採録)。

**曜日プロファイル検出 (2026-07-26、day_types.md §4)**: short ガードを越えたら、
まず日付集合から曜日規則を検出する — span 内の各曜日の出現率が `dow_on` (0.6)
以上の曜日を活性とし、活性曜日外の日の比率が `dow_stray_max` (0.1) 以下なら、
活性曜日集合をフラグと同じ写像でラベル化する (全平日→`weekday`、土日→`weekend`、
全曜日→`daily`、その他→`dow_XXXXXXX`)。全曜日活性 (=daily) の断定のみ、さらに
最小出現率 `dow_daily_min_cov` (0.9) を要求する (歯抜けの「ほぼ毎日」を毎日と
しない保守的閾値)。これにより「毎週水曜」をフラグで書いても日付列挙で書いても
同じ `dow_0010000` になる (根室交通 特別ダイヤ１で weekday 誤分類 → 平日タブ
4分裂の実例)。

検出できなければ従来の曜日分布多数決: `calendar_dates_majority` (0.8) 以上が
単一区分 (平日/土/日) に収まればその区分、収まらなければ `irregular`。
...
説明台帳の**分母**。2 世代間の機械的差分を決定的順序で全列挙し、`rawdiff_NNNNNN` の
...
| 行差分が閾値を超えたファイル | `rows_removed_bulk` / `rows_added_bulk` / `rows_changed_bulk` **各1件** (行数を old_value/new_value に十進文字列で保持) |

**行差分の集約 (bulk、2026-07-12)**: 1ファイルの行差分 (追加+削除+変更行) が
`[diff0].bulk_row_threshold` (既定 100,000) を超えたら、行単位の列挙をやめ集約
RawDiff にする。`file_added` を1件とするのと同じ粒度の規約の一部で、台帳の
網羅性は変わらない (集約 RawDiff も evidence か残差に必ず載る。台帳の分母では
**1件**と数え、行数は値と第1部ファイル表・FARE_CHANGED の quantification が
引き継ぐ)。検出ルールが行粒度の evidence を使うコアファイル
(`ROW_EVIDENCE_TABLES`: agency, stops, routes, trips, stop_times, calendar 系,
shapes, frequencies, transfers 等) は規模によらず集約しない。
実例: 鹿沼市リーバスの fare_rules 200万行 (docs/perf/kanuma_fare_bulk.md)。
あわせて主キー突合は「キー→行ダイジェスト」の2パス方式になり、メモリは
ファイル規模でなく差分規模に比例する。
...
2. なければ **基底名** (stop_name から「のりば・乗り場・番線・末尾の数字/丸数字/A-F」を
   ...
   ※ G6 (2026-09-16、i18n.md I4): **裸の1字指標 (A〜F・数字・丸数字) は、
   剥がした残りが ASCII 英数字で終わる場合は適用しない** — ラテン文字名では
   名前の一部であることが実測で支配的 (trimet 0/5・stm 0/8・rome 19/119 のみ
   乗り場)。国内479フィード全数スキャンでも挙動差14件は全て旧規則の
   実名破壊の修正 (「津名一宮IC」「アイセル21」等)。検証5フィードは差ゼロ
...
- 世代間リンク (M9 で内容主導化、docs/design/route_identity_review.md):
  1. 名称一致 → confidence 1.0 (`name_exact`)
  2. 名称不一致の対は**翻訳済み停留所集合 Jaccard** で判定する。翻訳 =
     停留所クラスタの世代間対応 (confidence ≥ `stop_link_min`、各旧クラスタの
     最良エッジ) による旧基底名 → 新基底名の写像。路線改称は停留所改称と
     同時に起きるのが普通で (施設名変更)、翻訳なしのパターン鍵比較は
     そこで共倒れする (名古屋 鳴.ワイ→鳴.メグ の実例)。
     - 受理 (`stops_translated`): Jaccard ≥ `link_min_similarity` (0.5)。
       route_id を共有する対は `id_bonus` (0.1) だけ閾値を緩める (弱い事前)
     - 候補 (`stops_candidate`): 受理未満で `link_floor` (0.3) 以上。
       イベント・ページ統合には使わず、レポートの「内容が類似する候補」注記のみ
  3. 受理エッジ (名称+内容) の**連結成分**を形で分類: 1:1=改称 / N:1=統合 /
     1:N=分割 / N:M=再編。関与 route_group 数が `max_component_groups` (6) を
     超えた成分は2段階で降格 — まず各旧 family の最良エッジだけ残す
     **best-match 間引き**で再分解 (コリドー共有による連鎖を断つ。朝日町
     宮崎境線↔市振線の実例)、それでも超過なら内容エッジを候補に全降格。
  4. **confidence 完全同点のタイブレークは同点証拠階層**
     (2026-07-28、docs/design/orientation.md): ①向きの一致度 (代表停車列の
     相対順序一致率 — 集合 Jaccard が潰す方向を回復。連続運行対・循環の
     方向分割で類似度1.0同点が系統的に出る) → ②名称類似 (弱い事前 —
     交差改称の防止) → ③未使用の新 family 優先 (同点の範囲でのみ新設/廃止
     という未説明を減らす) → ④辞書順 (決定性)。間引き (上記3) と
     ページ割付 (`page_family_maps`) が同一の比較器を共有する。
     非同点の選択には一切関与しない (朝日町・名古屋は同点0件で出力不変)。
     「新設/廃止ページ ∧ 完全一致級の対応相手の痕跡」は presentation の
     self_check (`level1_with_counterpart`) が常時検知する。
- この対応は trip 照合のブロッキング (§3)・A群イベント (§4.2)・レポートの
  ページ統合 (新世代を背骨に旧名称を注記) の共通土台になる。
...
2. 残りを **(route_group, day_type) のブロック内でコスト最小割当** (枝番 route 間の便移動に対応。2026-07-09 改訂)。ブロックの route_group は family 対応成分 (§2.2) で世代間に束ねたもの — 改称・統合・分割した路線の便も候補になる (M9):
...
既知の制限 (v1): family 対応が無い (受理閾値未満)・day_type が変わった便は
ブロックを跨げず removed+added になる (クリーン正解での recall 0.992、
未回収3件はすべて day_type 跨ぎ)。
...
| イベント | 検出条件 | evidence |
|---|---|---|
| ROUTE_RENAMED | family 対応成分 (§2.2) が 1:1 (名称のみ変化)。confidence = 停留所集合類似度 | 関与 route_id の routes.txt 行差分 + 改称 family 所属 trip の route_id field_changed |
| ROUTE_MERGED / ROUTE_SPLIT / ROUTE_RESTRUCTURED | 対応成分が N:1 / 1:N / N:M (v0.2.2、M9 で実装)。降格成分 (demoted) は発火しない | 同上 |
...
| ROUTE_DISCONTINUED | family が消滅し対応成分でも説明されない | routes 行 + **所属 trip の trips/stop_times 全行 (カスケード)** |
...
**未実装**: THROUGH_SERVICE_INTRODUCED / _DISCONTINUED。
...
trip が丸ごと入れ替わる形のパターン変化は C群 (SERVICE_*) 側で計上される。
...
(docs/design/service_days.md §2.2)。比較前に次を決定的に解決する:

1. 各スナップショットの有効窓 (feed_info → 世代メタ → calendar 全期間の順) の
   交差 = **共通窓**。交差しなければ何もしない (現行挙動)。
2. 共通窓を窓内の calendar 期間端点で**区間**に分割し、区間ごとの実効 active
   service 集合で**比較ユニット**に束ねる。ある unit が他の全 unit を両側とも
   包含する場合 (期間端が揃っていないだけの通常フィード) は全便比較 = 退化。
3. 真の同梱 (包含 unit なし) では、内容シグネチャが変化している unit のうち
   日数最大 (同数なら開始が遅い方) を **primary** とし、便世界から次を除外する:
   - **(a) 期限切れ・将来世代**: 共通窓に実効日を持たない service。ただし
     週次レギュラー型のみ (特定日型は特定日世界同士の比較を維持 — 名古屋の
     年次ペア)、かつ**後継条件**を満たすもの (下記)
   - **(b) 持ち越し世代**: primary に現れない active service。同じく
     レギュラー型+後継条件。厳密な内容同一は要求しない (持ち越し世代にも
     乗り場 ID 張り替え等の編集が入る — 伊勢市おかげバス prev_2 で実証)
   - **後継条件**: その service の路線 (family の世代間対応成分 = route_group
     ブロック粒度。route_id は世代サフィックスで張り替わるため使わない) の
     カバー率が同じ側の残存便世界で `[events.windows] carryover_coverage_min`
     (0.8) 以上。後継のない路線 (集約フィードで時刻表の掲載期限が路線ごとに
     違う佐賀の三瀬神埼線) は除外せず従来どおり比較する
4. 除外した service / 便の行差分 (calendar / calendar_dates / trips /
   stop_times / frequencies) と、同梱世代構造の calendar 行増減のうち他ルールが
   説明しなかったものを、カスケード最後尾の **GENERATION_SCOPE** (E群、info、
   confidence 1.0) が claim する。quantification: 比較窓・primary 期間・
   内容同一期間・除外 service/便数。JSON の `context.comparison_scope` にも
   同じ情報を出す (単一世代比較では null)。

検証: 桑名の同居スナップショット両方向で「全便半減/倍増」の誤説明 (各279件) が
消え explained 1.0000、伊勢市おかげバス prev_2 で偽増便72件が消えることを実測
(docs/verification/SD2_window_scope.md)。名古屋・臨港はイベント完全一致 (退化)。

既知の限界: (1) 同一 day_type 内の季節分割 (T3) はこの機構の対象外 (据え置き)。
(2) identity (M9) は全便で family 対応を作るため、持ち越し世代と名前一致して
しまうと改正側の再編が ROUTE_ADDED 等に落ちる (伊勢市の環状線 → 右回り/左回り)。
スコープ解決後の identity 再構築は将来課題。
...
用語注: 旧称「説明会計 (explanation accounting)」を 2026-07-10 に「説明台帳 (explanation ledger)」へ
改称した (概念・仕組みは不変)。JSON キー `accounting`・モジュール `events/accounting.py` 等の
コード識別子は安定インタフェースのため旧称のまま。
...
| `[diff0]` | |
|---|---|
| `bulk_row_threshold` | 1ファイルの行差分がこれを超えたら集約 RawDiff にする (0=無効、既定 100000) |

| キー | 既定値 | 用途 (参照節) |
|---|--:|---|
| load.day_types.calendar_dates_majority | 0.8 | day_type 判定の曜日多数決 (§0) |
| load.day_types.short_service_max_days | 10 | 短期間サービスの特定日分類 (§0) |
| load.day_types.min_flag_day_ratio | 0.5 | フラグ service の実効密度下限 — 未満は特定日 (§0 SD1) |
| events.windows.carryover_coverage_min | 0.8 | 世代除外の後継カバー率下限 (§4.5b SD2) |
...
| identity.route_family.link_min_similarity | 0.5 | family 対応の受理下限 (翻訳済み停留所集合 Jaccard、§2.2) |
| identity.route_family.id_bonus | 0.1 | route_id 共有対の閾値緩和 (§2.2) |
| identity.route_family.link_floor | 0.3 | 類似候補注記の保持下限 (§2.2) |
| identity.route_family.stop_link_min | 0.5 | 停留所翻訳に使うエッジの下限 (§2.2) |
| identity.route_family.max_component_groups | 6 | 対応成分の group 数上限 (§2.2) |
...
- THROUGH_SERVICE_* (A群。ROUTE_SPLIT / MERGED / RESTRUCTURED は M9 で実装済み)
