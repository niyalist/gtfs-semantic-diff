...
(exception_type=1) の曜日分布で判定する。`[load.day_types] calendar_dates_majority` (0.8)
以上が単一区分 (平日/土/日) に収まればその区分、収まらなければ `irregular`。
...
## 3. trip 照合 (events/tripdelta.py)

**trip_id の連続性は一切仮定しない。** 各 trip を内容署名
`(family, direction, day_type, 停車クラスタ基底名列, 全停留所の発着時刻列)` で表し:

| 分類 | 条件 | 主な用途 |
|---|---|---|
| exact | 署名完全一致の新旧ペア (同一 trip_id を優先ペアリング) | ID も同一なら「変更なし」 |
| churn | exact のうち trip_id が異なる対 | TECHNICAL_ID_CHURN |
| modified | 署名不一致だが同一 trip_id が両世代にある | 時刻修正 (C群)・経路変更 (B群) |
| removed / added | 上記いずれにも該当しない | 便数増減 (C群)、路線廃止・新設 (A群) |
...
対象は **modified** trip (同一 trip_id) のうち停車列 (基底名列) が変化したもの。
