...
- **SD2: 窓内区間対比較** — 共通窓を calendar 期間端点で区間分割し、区間ごと
  に比較。内容同値区間は「変化なし」に畳む。窓外・同値の claim イベントは
  残差精査 → ontology 採録の手続きで追加。
  DoD: 桑名両方向 (偽減便/偽増便 各279件が消え、実測2で「7/11 改正」の中身が
  正しく出る)・同居×同居の圧縮・伊勢市おかげバス prev_2。既存フィード回帰不変。
