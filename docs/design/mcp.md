...
| worker の同時実行バースト (上限内でも瞬間 N×3GB) | 予約同時実行なし (非同期呼び出し) | **G2 【恒久化 2026-09-17 (XL3 前半)】**: アカウント同時実行クォータの 10→1000 引き上げ承認後、worker に reserved concurrency **4** を設定 (バーストコスト上限)。非同期呼び出しは Lambda が自動キューするため5件目以降は自然に直列化される。api/mcp は未予約枠 996 を共有 — worker 起因の飢餓は解消 |
