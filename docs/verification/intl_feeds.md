...
| IN-3 | P2/I4 (**前半解消 2026-07-24**) | shapes・stop_times 由来の巨大 RawDiff (660万〜1520万) — メモリと HTML サイズに直結。**メモリ側は解消: HTML バンドルの遅延直列化+逐次書き出しで prt フルパス 5.15→2.17GB (HTML byte 同一、docs/perf/P2_html_memory.md)。prt 級は Lambda 3008MB に収まる見込み。** 残: HTML 自体のサイズ (prt 478MB — rawdiffs 全件埋め込み)。hosted bundle 化 (X1) / 件数上限 / サイズガードの製品判断待ち |
