...
正確な仕様の定義。導入と使い方の案内は [README.ja.md](README.ja.md)。
...
ジョブ投入には日次の計算回数上限があり、超過時は 429 (Retry-After 付き)。
キャッシュ済みペア (計算済みの再取得) は消費しない。制限値は予告なく
変わり得る。大量の機械的投入はせず、まとまった処理は CLI をローカルで。
