...
const DICT = {
  ja: {
    ...
    fo_scope_feed: (side, text) => `${side}データ: ${text}`,
    fo_scope_version: (v) => `feed_version「${v}」`,
    fo_scope_no_version: "feed_version 記載なし",
    fo_scope_no_feedinfo: "feed_info なし (任意ファイル)",
    cmp_title: "比較の概要",
    cmp_data: "データ:",
    cmp_cond: "比較の条件:",
    cmp_result: "結果:",
    cmp_actual: (p) => `(実データは ${p})`,
    cmp_no_window: "期間情報が読めない・または長期のため、日付単位の照合は行っていない (時刻表全体の比較)",
    cmp_scope_primary: (p) => `同梱世代を検出 — 比較は ${p} の世代同士で実施`,
    cmp_scope_switch: (d) => `同梱世代を検出 — 運行の切替日 ${d} を挟み、旧データの改正前 × 新データの改正後を比較`,
    cmp_no_change: "重なる期間内に運行内容の変化はない",
    cmp_switch: (d) => `運行の切替日: ${d} (この日以降、データ終端まで運行内容が異なる)`,
    cmp_all_changed: (a, b) => `重なる期間 (${a}〜${b}) の全日で運行内容が異なる (切替日はこの期間より前の可能性)`,
    cmp_span: (a, b) => `${a}〜${b} は運行内容が異なる (期間限定の変化)`,
    cmp_more_runs: (n) => `ほか ${n} 区間で運行内容が異なる`,
    cmp_total: (n) => `(内容が異なる日: 計${n}日)`,
    fo_scope_excluded_old: (s, t) =>
      `旧側の比較対象外: ${s} service・${t}便 (期限切れ・持ち越し世代)`,
    fo_scope_excluded_new: (s, t) =>
      `新側の比較対象外: ${s} service・${t}便 (期限切れ・持ち越し世代)`,
    rp_special_dates: (s) => `この「特定日」の${s}`,
    sdn_overlap: (a, b) => `両方が重なる期間: ${a}〜${b}`,
    sdn_no_overlap: "期間が重ならない2時点の比較 (日付単位の照合は行わない)",
    sdn_swap: "曜日と異なるダイヤで走る日",
    sdn_no_service: "運行のない日",
    sdn_none: "なし",
    sdn_more: (n) => ` ほか${n}区間`,
    sdn_days: (n) => `計${n}日`,
    ...
    former_names: "旧名称",
    related_names: "同じ再編に関わる旧路線 (便数は各対応ページに計上)",
    ...
    fdb_unexplained: "未説明",
    fdb_sample_rest: (n) => `…ほか ${n} 件 (Web 表示はサンプルのみ。全件は生データを参照)`,
    ...
    total_days: (n) => `全${n}日`,
    cal_both: "新旧とも運行する日",
    cal_old_only: "旧で運行・新世代データで上書きされ消えた日 (取り消し線)",
    cal_old_outside: "旧世代のみが記録する日 (新世代データの範囲外 — 枠線)",
    cal_new_only: "新のみ (新たな運行日 — 下線)",
    cal_old_side: "旧世代の運行日 (取り消し線)",
    cal_new_side: "新世代の運行日 (下線)",
    cell_same_content: "内容同一 — 運行日の変更",
    cell_flow: "便対応で接続 — 時刻等の変更として説明",
    cell_dates_more: (n) => `ほか (全${n}日)`,
    gross_trips: (n) => `のべ${n}便`,
    cell_mixed: "⚠ この区分は日により運行構成が異なる (便数はのべ — 各日の便数はデータ上一定でない)",
    cell_mixed_shared: (d) => ` 複数ダイヤが重なる日: ${d}`,
    fdb_columns: "列変更",
  ...
  en: {
    ...
    fo_scope_feed: (side, text) => `${side} data: ${text}`,
    fo_scope_version: (v) => `feed_version “${v}”`,
    fo_scope_no_version: "no feed_version",
    fo_scope_no_feedinfo: "no feed_info (optional file)",
    cmp_title: "Comparison overview",
    cmp_data: "Data:",
    cmp_cond: "Comparison conditions:",
    cmp_result: "Findings:",
    cmp_actual: (p) => `(actual data: ${p})`,
    cmp_no_window: "No usable period information (or too long a span) — compared the timetables as a whole, without date-by-date matching",
    cmp_scope_switch: (d) => `Bundled generations detected — comparing pre-switch (old) vs post-switch (new) across the switchover on ${d}`,
    cmp_scope_primary: (p) => `Bundled generations detected — compared the generations covering ${p}`,
    cmp_no_change: "No change in service content within the overlapping period",
    cmp_switch: (d) => `Service switchover date: ${d} (content differs from this day to the end of the data)`,
    cmp_all_changed: (a, b) => `Service content differs on every day of the overlap (${a}–${b}); the switchover may predate this period`,
    cmp_span: (a, b) => `Service content differs during ${a}–${b} (bounded change)`,
    cmp_more_runs: (n) => `${n} more period(s) with differing content`,
    cmp_total: (n) => `(${n} day(s) differ in total)`,
    fo_scope_excluded_old: (s, t) =>
    ...
    rp_special_dates: (s) => `The “special dates” here: ${s}`,
    sdn_overlap: (a, b) => `Period covered by both: ${a}–${b}`,
    sdn_no_overlap: "The two feeds cover non-overlapping periods (no date-by-date comparison)",
    sdn_swap: "Days running a different schedule than the weekday suggests",
    sdn_no_service: "Days with no service",
    sdn_none: "none",
    sdn_more: (n) => ` +${n} more`,
    sdn_days: (n) => `${n} day(s)`,
    ...
    related_names: "Old routes in the same restructuring (trips counted on their own pages)",
    ...
    fdb_sample_rest: (n) => `…and ${n} more (web shows a sample; see raw data for all rows)`,
    ...
    total_days: (n) => `${n} days`,
    cal_both: "runs in both generations",
    cal_old_only: "ran in old, overridden by the new data (strikethrough)",
    cal_old_outside: "recorded only by the old generation (outside new data's coverage — outlined)",
    cal_new_only: "new only (added — underline)",
    cal_old_side: "old-generation dates (strikethrough)",
    cal_new_side: "new-generation dates (underline)",
    ...
    gross_trips: (n) => `${n} trips total`,
    cell_mixed: "⚠ service composition varies by date (counts are totals, not per-day)",
    cell_mixed_shared: (d) => ` dates with overlapping schedules: ${d}`,
